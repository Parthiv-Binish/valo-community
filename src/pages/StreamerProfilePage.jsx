...
export default function StreamerProfilePage() {
  ...
  const alsoLive = useMemo(() => {
    if (!streamer) return []
    const others = streamers.filter((s) => s.isLive && s.dbId !== streamer.dbId)
    if (streamer.language) {
      others.sort(
        (a, b) => Number(b.language === streamer.language) - Number(a.language === streamer.language)
      )
    }
    return others.slice(0, ALSO_LIVE_LIMIT)
  }, [streamers, streamer])

  // Page title / description (the route-level default is set by RouteMeta first).
  useEffect(() => {
    if (!streamer) return
    document.title = `${streamer.channelName} – VALORANT streamer | ${SITE}`
            ...
            {alsoLive.length > 0 && (
              <section className="space-y-4" aria-label="Also live now">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-8">
                  {alsoLive.map((s) => (
                    <StreamCard key={s.dbId || s.channelId} streamer={s} />
                  ))}
                </div>
              </section>
            )}
...
function ProfileHeader({ streamer }) {
  ...
  const initial = (streamer.channelName || '?').charAt(0).toUpperCase()
  const watchUrl = streamer.isLive ? streamer.streamUrl : streamer.channelUrl
  ...
  return (
    <header className="relative overflow-hidden rounded-2xl border border-valo-border bg-valo-card">
      ...
      <div
        ...
        style={{ background: `radial-gradient(600px 200px at 0% 0%, ${accent}22, transparent 70%)` }}
      />
        ...
        <div className="relative shrink-0 self-start">
          {streamer.avatar ? (
            <img
              ...
              className={`w-24 h-24 rounded-full object-cover border-4 ${streamer.isLive ? 'border-valo-red' : 'border-white/10'}`}
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div className="space-y-1.5">
            <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
              ...
              {streamer.verified && (
                <svg className="w-5 h-5 shrink-0 text-[#3ea6ff]" viewBox="0 0 24 24" fill="currentColor" aria-label="Verified">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className={isKick ? 'platform-badge-kick' : 'platform-badge-yt'}>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              {streamer.isLive ? 'Watch live' : 'Visit channel'} on {platformLabel(streamer.platform)}
  ...
  return (
    ...
    >
      ...
      <div className="md:col-span-2 flex flex-col justify-center gap-4 min-w-0">
        <div>
          <p className="text-[10px] font-mono uppercase tracking-widest text-valo-red mb-1.5">Streaming now</p>
          <h2 className="font-display font-bold text-lg text-white leading-snug line-clamp-3">
            {streamer.title && streamer.title !== 'Live Stream' ? streamer.title : `${streamer.channelName} is live`}
          </h2>
        </div>

        <dl className="grid grid-cols-2 gap-3">
          <Stat label="Watching" value={formatViewerCount(streamer.viewerCount || 0)} />
          <Stat label="Live for" value={liveFor || '—'} />
          {streamer.category && <Stat label="Category" value={streamer.category} span />}
        </dl>
      </div>
    </section>
  )
...
function OfflinePanel({ streamer, slots }) {
  return (
    ...
    >
      <span className="w-2.5 h-2.5 mt-1.5 rounded-full bg-neutral-600 shrink-0" aria-hidden="true" />
...
function UsuallyLive({ slots, name }) {
  return (
    <section className="space-y-4" aria-label="Usually live">
      <SectionTitle>Usually live</SectionTitle>
        ...
        {slots.map((slot) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <span className="text-sm font-display font-semibold text-white">{slot.label}</span>
              <span className="text-xs font-mono text-valo-muted">{slot.probability}%</span>
            </div>
            <div className="h-1 rounded-full bg-neutral-800 overflow-hidden" aria-hidden="true">
              <div className="h-full rounded-full bg-valo-red" style={{ width: `${Math.min(100, slot.probability)}%` }} />
            </div>
          </li>
        ))}
      </ul>
...
function ShareButton({ streamer }) {
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}${profilePath(streamer)}`
    const title = `${streamer.channelName} on ${SITE}`
...
function Stat({ label, value, span = false }) {
  return (
    <div className={`rounded-lg bg-black/30 border border-white/5 px-3 py-2 min-w-0 ${span ? 'col-span-2' : ''}`}>
      <dt className="text-[10px] font-mono uppercase tracking-widest text-valo-muted">{label}</dt>
      <dd className="font-display font-bold text-white truncate">{value}</dd>
    </div>
  )
}
...
function SectionTitle({ children }) {
  return (
    <div className="flex items-center gap-2.5 select-none">
      <h2 className="font-display font-black text-xs uppercase tracking-widest text-neutral-400">{children}</h2>
...
function ProfileSkeleton() {
  return (
    <div className="space-y-6" role="status" aria-label="Loading streamer profile">
...
function NotFound() {
  return (
    <div className="flex flex-col items-center text-center py-20 rounded-2xl border border-valo-border bg-valo-card px-6">
      <div className="w-14 h-14 rounded-xl bg-neutral-900/60 border border-neutral-800 flex items-center justify-center mb-4 text-xl" aria-hidden="true">
        🔎
      </div>
      <h1 className="font-display font-black text-lg text-white mb-1">Streamer not found</h1>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        <Link to="/" className="valo-btn">Browse streamers</Link>
        <Link to="/submit" className="valo-btn-ghost">Suggest a streamer</Link>
      </div>
    </div>
  )
}
